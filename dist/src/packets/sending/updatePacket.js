...
class UpdatePacket extends sendingPacket_1.SendingPacket {
    constructor(p) {
        super(p);
        this.msgType = constants_1.MsgType.Update;
    ...
    serialize(stream) {
        ...
        if (game.gasCircleDirty || p.fullUpdate)
            valuesChanged += 16;
        if (game.playerInfosDirty)
            valuesChanged += 32;
        if (game.deletedPlayers.size)
            valuesChanged += 64;
        if ( /* game.dirtyStatusPlayers.length || */p.fullUpdate)
            valuesChanged += 128;
        ...
        if (p.planesDirty)
            valuesChanged += 4096;
        if (p.airstrikeZonesDirty)
            valuesChanged += 8192;
        if (p.mapIndicatorsDirty)
            valuesChanged += 16384;
        ...
        if (p.actionDirty) {
            stream.writeFloat(p.actionItem.duration - ((p.actionItem.useEnd - Date.now()) / 1000), 0, constants_1.Constants.ActionMaxDuration, 8);
            stream.writeFloat(p.actionItem.duration, 0, constants_1.Constants.ActionMaxDuration, 8);
        ...
        if (game.deletedPlayers.size > 0) {
            ...
            for (const player of game.deletedPlayers)
        ...
        if (p.emotes.size) {
            ...
            for (const emote of p.emotes) {
                ...
                stream.writeGameType(emote.type); // hack unsafe
                stream.writeGameType(0); // Item type
                stream.writeBoolean(emote.isPing);
                if (emote.isPing)
                    stream.writeVec(emote.position, 0, 0, 1024, 1024, 16);
